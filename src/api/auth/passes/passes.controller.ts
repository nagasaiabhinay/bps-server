...
const createNewPass = async (req: Request, res: Response) => {
    try {
        ...
        const firebaseUID = decodedToken?.user?.firebaseUID;
        ...
        if (!category || !details || !startDate || !endDate || !passType || !paymentID) {
            return res.status(400).json({
                ...
                required: `Field "${
                    !category ? 'category' : !details ? 'details' : !startDate ? 'startDate' : !endDate ? 'endDate' : !passType ? 'passType' : !paymentID ? 'paymentID' : ''
                }" is missing`
            });
        }
